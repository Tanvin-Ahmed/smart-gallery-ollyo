...
>({
  ...
  const copyItem = React.useCallback(
    (sourceIndex: number) => {
      ...
      //! store the actual size of the copy element
      sourceElementActualInfoRef.current = {
  ...
  const listeners = useDrag({
    ...
    onMove: ({ pointInWindow }) => {
      ...
      // ! change the size of the dropped target placeholder according to the target element size
      if (itemsRect.current[targetIndex]) {
        dropTargetLogic.show?.(itemsRect.current[targetIndex]);
      }

      //! ensure the copy has the same size of targeted position size
      const copy = targetRef.current;
      const targetedRect = itemsRect.current[targetIndex];
      if (copy && targetedRect) {
        copy.style.width = `${targetedRect.width}px`;
        copy.style.height = `${targetedRect.height}px`;
      }
      ...
      for (let index = 0; index < itemsRef.current.length; index += 1) {
        ...
        if (
          ...
          const newIndex = isMovingRight ? index - 1 : index + 1;
          const nextItemRects = itemsRect.current[newIndex];
          if (nextItemRects) {
            ...
            // ! handle the size of currentItem according to the size of the position where it will be placed
      ...
      if (sourceIndex !== undefined) {
        ...
        if (targetIndex !== undefined) {
          if (sourceIndex !== targetIndex) {
            //! before sort we ensure that the size of the source element is same as the target element
            const srcElement = itemsRef.current[sourceIndex];
            const targetElementRect = itemsRect.current[targetIndex];
            srcElement.style.height = `${targetElementRect.height}px`;
            srcElement.style.width = `${targetElementRect.width}px`;
